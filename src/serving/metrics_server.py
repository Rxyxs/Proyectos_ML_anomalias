...
Servidor con threads y backlog ampliado (Día 10): la prueba de carga
concurrente del Día 10 midió `/metrics` con 15 workers scrapeando en
paralelo y encontró p95=610ms, p99>1s -- contra scrapes individuales de
unos pocos ms. La hipótesis obvia ("el servidor atiende un request a la
vez") resultó ser INCOMPLETA: agregar `ThreadingMixIn` por sí solo casi no
cambió los números (p95=528ms) en una comparación A/B directa. Perfilando
por nivel de concurrencia (1, 3, 5, 15 workers) el salto aparece justo entre
5 y 15 -- exactamente el valor por defecto de
`socketserver.BaseServer.request_queue_size` (5): con más de 5 conexiones
entrantes simultáneas, el backlog TCP del `listen()` subyacente se llena, y
el SO retiene el resto hasta que se libera un lugar, agregando cientos de
milisegundos de espera antes de que el proceso Python vea siquiera la
conexión. Confirmado subiendo `request_queue_size` a 128: p95 bajó de
~600ms a **71ms** (~8x), medido en el mismo escenario. `ThreadingMixIn`
...
class ThreadingWSGIServer(ThreadingMixIn, WSGIServer):
    ...
    que el default de 5 -- ver la nota del Día 10 en el docstring del
    módulo: el backlog, no la falta de threads, era la causa real de la
    latencia bajo carga concurrente."""
